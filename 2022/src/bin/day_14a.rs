...
impl Map {
    pub fn add_floor(&mut self, depth: u16, floor: impl Iterator<Item = u16>) {
        ...
        self.floor_depth = self.floor_depth.max(depth);
    }

    pub fn add_wall(&mut self, x: u16, wall: impl Iterator<Item = u16>) {
        wall.for_each(|depth| {
            self.occupied.insert((x, depth));
        });
    }
    ...
    pub fn add_sand(&mut self) -> bool {
        let mut pos = (500, 0);
        assert!(!self.occupied.contains(&pos));
        loop {
            if pos.1 >= self.floor_depth {
                break false;
            }
