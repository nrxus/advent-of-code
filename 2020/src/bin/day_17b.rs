...
fn solve(initial: &str) -> usize {
    ...
    (0..6).for_each(|_| {
        actives = (min.0 - 1..=max.0 + 1)
            ...
            .flat_map(|(x, y, z)| (min.3 - 1..=max.3 + 1).map(move |w| (x, y, z, w)))
            .filter_map(|(x, y, z, w)| {
                let active_count = neighbors((x, y, z, w))
        ...
        min.3 = actives.iter().map(|p| p.3).min().unwrap();
    });
...
fn neighbors((x, y, z, w): (i32, i32, i32, i32)) -> impl Iterator<Item = (i32, i32, i32, i32)> {
    (x - 1..=x + 1)
        ...
        .flat_map(move |(x, y, z)| (w - 1..=w + 1).map(move |w| (x, y, z, w)))
